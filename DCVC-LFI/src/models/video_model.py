...
class DMC(CompressionModel):
    ...
    def encode_decode(self, x, dpb, output_path=None,
        ...
        encoded = self.forward_one_frame(x, dpb, rate_idx=[mv_y_q_scale, y_q_scale])
        result = {
            "dpb": encoded['dpb'],
    ...
    def forward_one_frame(self, x, dpb, rate_idx):
        ref_frame = dpb["ref_frame"]
        if isinstance(rate_idx, int):
            # 处理单个int的情况
            curr_mv_y_q = self.get_curr_mv_y_q(self.mv_y_q_scale[rate_idx])
            curr_y_q = self.get_curr_y_q(self.y_q_scale[rate_idx])
        elif isinstance(rate_idx, list) and len(rate_idx) == 2:
            # 处理两个float的列表情况
            curr_mv_y_q = self.get_curr_mv_y_q(rate_idx[0])
            curr_y_q = self.get_curr_y_q(rate_idx[1])
    ...
    def forward_motion(self, x, dpb, rate_idx):
        ...
        mv_hat = self.mv_decoder(mv_y_hat)
        context1, context2, context3, warp_frame = self.motion_compensation(dpb, mv_hat)
        # discrete_warpframe = flow_warp(dpb["ref_frame"], mv_hat) #
