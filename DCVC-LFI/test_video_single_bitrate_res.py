...
-w 1
--write_stream 1
--output_path DCVC-LFI_q4_res_q3_P_Mask_0.5dB_SnakeRev_ALL.json
...
from src.utils.png_reader_res import PNGReader
...
def parse_args():
    ...
    parser.add_argument('--rate_idx', type=int, default=4)
...
def run_test(video_net, i_frame_net, args, device):
    ...
    with torch.no_grad():
        all_image_files = src_reader.get_all_image_files()
        for frame_idx, img_file in enumerate(all_image_files[:frame_num]):
            frame_start_time = time.time()

            rgb = src_reader.read_frame_by_name(img_file)
            if rgb is None:
                break
            ...
            if verbose >= 2:
                print(f"frame {frame_idx} ({img_file}), {frame_end_time - frame_start_time:.3f} seconds,",
                      f"bits: {bits[-1]:.3f}, PSNR: {psnrs[-1]:.4f}, MS-SSIM: {msssims[-1]:.4f} ")

            if save_decoded_frame:
                save_path = os.path.join(args['decoded_frame_folder'], img_file)
...
def encode_one(args, device):
    ...
    if args['save_decoded_frame']:
        ...
        model_dir = Path(args['model_path']).parts[1]
        # decoded_frame_folder = Path('./experiments') / model_dir / Path('save_png') / Path(args['ds_name']) / Path(str(args['rate_idx'])) / sub_dir_name
        decoded_frame_folder = Path('./experiments') / model_dir / Path('save_png') / Path(args['ds_name']) / sub_dir_name
...
def main():
    ...
    for rate_idx in range(rate_num):
        ...
        for ds_name in log_result:
            for seq_name in log_result[ds_name]:
                if f"{rate_idx:03d}" in log_result[ds_name][seq_name]:
                    res = log_result[ds_name][seq_name][f"{rate_idx:03d}"]
                    res['frame_num'] = res['i_frame_num'] + res['p_frame_num']
                    total_bpp += res['ave_all_frame_bpp'] * res['frame_num']
                    total_psnr += res['ave_all_frame_psnr'] * res['frame_num']
                    total_msssim += res['ave_all_frame_msssim'] * res['frame_num']
                    total_frames += res['frame_num']
                    scene_count += 1
        ...
        if scene_count > 0:
            ...
            msssim_list.append(avg_msssim)

    final_result = {
        'bpp': bpp_list,
        'psnr': psnr_list,
        'msssim': msssim_list,
        'detailed_results': log_result
    }
    ...
    with open(args.output_path, 'w') as fp:
        dump_json(final_result, fp, float_digits=6, indent=2)
    ...
    print("\nAverage metrics per rate:")
    print(f"bpp = {bpp_list}")
    print(f"psnr = {psnr_list}")
    print(f"msssim = {msssim_list}")
